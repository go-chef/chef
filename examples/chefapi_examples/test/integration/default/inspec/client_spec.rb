...
describe command('/go/src/chefapi_test/bin/client') do
  its('stderr') { should match(%r{^Couldn't recreate client client1.  POST https://localhost/organizations/test/clients: 409}) }
  its('stderr') { should_not match(/error|no such file|cannot find|not used|undefined/) }
  its('stdout') { should match(%r{^List initial clients test-validator => https://localhost/organizations/test/clients/test-validator}) }
  its('stdout') { should match(/^Define client1 \{Name:client1 ClientName: Validator:false Admin:false CreateKey:true\}/) }
  its('stdout') { should match(%r{^Added client1 &\{Uri:https://localhost/organizations/test/clients/client1 ChefKey:\{Name:default PublicKey:-----BEGIN PUBLIC KEY-----MIIB.*ExpirationDate:infinity Uri:.*PrivateKey:-----BEGIN RSA PRIVATE KEY}) }
  its('stdout') { should match(%r{^Added client2 &\{Uri:https://localhost/organizations/test/clients/client2 ChefKey:\{Name: PublicKey: ExpirationDate: Uri: PrivateKey:\}\}}) }
  ...
  its('stdout') { should match(%r{^List clients after cleanup test-validator => https://localhost/organizations/test/clients/test-validator}) }
end
