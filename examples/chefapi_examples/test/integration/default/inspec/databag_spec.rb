...
describe command('/go/src/chefapi_test/bin/databag') do
  its('stderr') { should match(%r{^Issue recreating databag1. POST https://localhost/organizations/test/data: 409}) }
  its('stderr') { should match(%r{^Issue getting nothere. GET https://localhost/organizations/test/data/nothere: 404}) }
  its('stderr') { should_not match(/error|no such file|cannot find|not used|undefined/) }
  its('stdout') { should match(/^List initial databags\s*$/) }
  its('stdout') { should match(%r{^Added databag1 \&\{https://localhost/organizations/test/data/databag1\}}) }
  its('stdout') { should match(%r{^List databags after adding databag1 databag1 => https://localhost/organizations/test/data/databag1}) }
  its('stdout') { should match(/^Create databag1::item1 \<nil\>/) }
  its('stdout') { should match(/^Update databag1::item1 \<nil\>/) }
  its('stdout') { should match(%r{^List databag1 items item1 => https://localhost/organizations/test/data/databag1/item1}) }
