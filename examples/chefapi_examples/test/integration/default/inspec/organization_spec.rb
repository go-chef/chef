...
describe command('/go/src/chefapi_test/bin/organization') do
  ...
  its('stderr') { should match(/^Issue creating org: {org1 organization1 } POST https:\/\/localhost\/organizations: 409$/) }
  ...
  its('stdout') { should match(/^List organizations After adding org1 and org2 map(?=.*org2:https:\/\/localhost\/organizations\/org2)(?=.*test:https:\/\/localhost\/organizations\/test)(?=.*org1:https:\/\/localhost\/organizations\/org1)/) }
  ...
  its('stdout') { should match(/^List organizations after deleting org2 map(?=.*test:https:\/\/localhost\/organizations\/test)(?=.*org1:https:\/\/localhost\/organizations\/org1)/) }
  its('stdout') { should match(/^Result from deleting org1 <nil>/) }
  its('stdout') { should match(/^List organizations after cleanup map\[test:https:\/\/localhost\/organizations\/test\]/) }
end
