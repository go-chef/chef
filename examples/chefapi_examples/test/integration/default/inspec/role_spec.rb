...
describe command('/go/src/chefapi_test/bin/role') do
  its('stderr') { should match(%r{^Issue recreating role1: POST https://localhost/organizations/test/roles: 409}) }
  its('stderr') { should match(%r{^Issue getting nothere: GET https://localhost/organizations/test/roles/nothere: 404}) }
  its('stderr') { should_not match(/error|no such file|cannot find|not used|undefined/) }
  its('stdout') { should match(%r{^Added role1 uri => https://localhost/organizations/test/roles/role1}) }
  its('stdout') { should match(%r{^List roles after adding role1 role1 => https://localhost/organizations/test/roles/role1}) }
