...
describe command('/go/src/chefapi_test/bin/userkeys') do
  ...
  its('stdout') { should match(%r{^List initial user usr1 keys \[\{KeyName:default Uri:https://localhost/users/usr1/keys/default Expired:false\}\]}) }
  its('stdout') { should match(%r{^List initial user usr2 keys \[\{KeyName:default Uri:https://localhost/users/usr2/keys/default Expired:false\}\]}) }
  its('stdout') { should match(/^List initial user usr3 keys \[\]/) }
  its('stdout') { should match(%r{^Add usr1 key \{KeyName: Uri:https://localhost/users/usr1/keys/newkey Expired:false\}}) }
  its('stdout') { should match(/^List after add usr1 keys \[\{(?=.*newkey)(?=.*default).*\}\]/) }
  its('stdout') { should match(%r{^Add usr3 key \{KeyName: Uri:https://localhost/users/usr3/keys/default Expired:false\}}) }
