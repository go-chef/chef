...
describe command('/go/src/github.com/go-chef/chef/testapi/bin/node') do
  ...
  its('stdout') { should match(/^Define node1 {Name:node1 Environment:_default.*Chef::Node RunList:\[pwn\]/) }
  ...
  its('stdout') { should match(/^Get node1 {Name:node1 Environment:_default ChefType:node AutomaticAttributes:map\[attr:value\] NormalAttributes:map\[\] DefaultAttributes:map\[\] OverrideAttributes:map\[\] JsonClass:Chef::Node RunList:\[recipe\[pwn\]\] PolicyName: PolicyGroup/) }
